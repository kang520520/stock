...
const INDUSTRY_GROUPS = {
    ...
    "🧪 電子上游": ["電子上游-IC-設計", "電子上游-IC-代工", "電子上游-IC-製造", "電子上游-IC-封測", "電子上游-IC-通路", "電子上游-IC-其他", "電子上游-IC-DRAM製造", "電子上游-DRAM銷售", "電子上游-PCB-材料設備", "電子上游-PCB-製造", "電子上游-被動元件", "電子上游-連接元件", "電子上游-LED及光元件"],
...
const getStockButtons = (code) => {
    return Markup.inlineKeyboard([
        [Markup.button.url(`📲 籌碼K線 App`, `cmchipk://stock/${code}`)],
...
const makeKeyboard = (userId, group = null) => {
    ...
    if (!group) {
        ...
        btns.push([Markup.button.callback(`🚀 執行選股 (${filters.length}項)`, 'run'), Markup.button.callback('🧹 清空', 'reset')]);
        return Markup.inlineKeyboard(btns);
    }
...
bot.on('callback_query', async (ctx) => {
    ...
    try {
        if (data === 'main' || data === 'reset') {
            ...
            await ctx.editMessageText('請選擇分類進行篩選：', makeKeyboard(userId));
        } else if (data === 'run') {
            ...
            await ctx.reply(`🎯 找到 ${res.length} 支股票 (顯示前 15 支)：`);
            // 分批發送，避免爆流量
            for (const s of res.slice(0, 15)) {
                ...
                const text = `<b>[${code}] ${name}</b>\n價: ${price} (${pv > 0 ? '+' : ''}${pv}%)\n產業: ${fuzzyGet(s, "產業") || "未分類"}`;
                await ctx.replyWithHTML(text, getStockButtons(code));
            ...
            await ctx.reply(`✅ 已更新條件：${val}`, makeKeyboard(userId));
        } else if (data.startsWith('set_')) {
            const param = data.replace('set_', '');
            state.stage = param;
            if (["產業", "籌碼力道"].includes(param)) {
                state.tempOp = "include";
                await ctx.editMessageText(`設定 [${param}]：`, makeMultiSelectKeyboard(userId, param));
            } else {
                await ctx.editMessageText(`設定條件：`, Markup.inlineKeyboard([
            ...
            await ctx.editMessageText(`選擇產業：`, makeMultiSelectKeyboard(userId, "產業", data.replace('indgroup_', '')));
        } else if (data.startsWith('op_')) {
            const p = data.split('_'); state.stage = p[1]; state.tempOp = p[2];
            if (QUICK_OPTIONS[state.stage]) await ctx.editMessageText(`勾選選項：`, makeMultiSelectKeyboard(userId, state.stage));
            else await ctx.reply(`💬 請輸入 [${state.stage}] 的數值：`);
            ...
            state.tempSelected = []; await ctx.reply(`✅ 條件已加入`, makeKeyboard(userId));
        }
...
bot.on('text', async (ctx) => {
    ...
    if (!userStates[userId]) userStates[userId] = { params: [] };
    const state = userStates[userId];
    ...
    if (state.stage) {
        ...
        state.stage = null; return ctx.reply(`✅ 已設定條件`, makeKeyboard(userId));
    }
    ...
    if (text.toLowerCase().startsWith('p')) {
        ...
        if (res.length > 0) {
            for (const s of res.slice(0, 5)) {
                const code = String(fuzzyGet(s, "代碼")).replace(/[ "]/g, "");
                await ctx.replyWithHTML(`<b>[${code}] ${fuzzyGet(s, "名稱")}</b>\n價: ${fuzzyGet(s, "價格")}`, getStockButtons(code));
            }
